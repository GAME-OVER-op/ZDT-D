...
Рекомендуется обновиться для максимальной совместимости. Можно пропустить, но часть функций может быть недоступна, а также возможны ошибки из-за разницы API.""".trimIndent()
  ...
  private suspend fun checkZapretInternal() {
    ...
    val (latestVer, downloadUrl) = latest
    val updAvail = compareVersions(installed, latestVer) < 0
    ...
    _programUpdates.update { st ->
      st.copy(
        zapret = st.zapret.copy(
          ...
          statusText = if (updAvail) "Update available" else "Up to date",
          errorText = null,
        )
      )
    }
  ...
  private suspend fun updateZapretInternal() {
    ...
    if (_programUpdates.value.zapret.latestVersion.isNullOrBlank() || _programUpdates.value.zapret.downloadUrl.isNullOrBlank()) {
      checkZapretInternal()
    }
    val st0 = _programUpdates.value.zapret
    val url = st0.downloadUrl
    val latestVer = st0.latestVersion
    if (url.isNullOrBlank() || latestVer.isNullOrBlank()) return

    _programUpdates.update { st ->
      st.copy(zapret = st.zapret.copy(updating = true, progressPercent = 0, errorText = null, statusText = "Downloading…"))
    }

    val zipFile = File(ctx.cacheDir, "zapret_latest.zip")
    ...
    val updAvail = if (!installed.isNullOrBlank()) compareVersions(installed, latestVer) < 0 else false
    _programUpdates.update { st ->
      st.copy(
        zapret = st.zapret.copy(
          updating = false,
  ...
  private suspend fun updateZapret2Internal() {
    if (!requireServiceStoppedForUpdates()) return
    if (_programUpdates.value.zapret2.latestVersion.isNullOrBlank() || _programUpdates.value.zapret2.downloadUrl.isNullOrBlank()) {
      checkZapret2Internal()
    }
    ...
    val zipFile = File(ctx.cacheDir, "zapret2_latest.zip")
