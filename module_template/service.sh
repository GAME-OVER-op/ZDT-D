...
# Conntrack: увеличить время "stream" для долгих UDP сессий (games/voice/WebRTC)
apply net.netfilter.nf_conntrack_udp_timeout_stream 300

# Conntrack: оставить короткие UDP транзакции (DNS) маленькими
apply net.netfilter.nf_conntrack_udp_timeout 30

# Сетевые буферы: увеличить max/wmem для более стабильного пинга при burst
apply net.core.rmem_max 2621440      # 2.5MB
apply net.core.wmem_max 2621440
apply net.core.rmem_default 262144   # 256KB
apply net.core.wmem_default 262144

# Очереди/accept backlog: чтобы не терять входящие пакеты при пиках
apply net.core.netdev_max_backlog 2500
apply net.core.somaxconn 1024

# PMTU/мереджинг: авто-пробное восстановление MTU (помогает проблемам с фрагментацией)
apply net.ipv4.tcp_mtu_probing 1

# Повторное использование TIME_WAIT (клиентская оптимизация)
apply net.ipv4.tcp_tw_reuse 1

# Расширить диапазон локальных портов (если часто много исходящих соединений)
apply net.ipv4.ip_local_port_range "10240 65535"

# Позволяет избегать slow-start после паузы (быстрее возобновление потоков)
apply net.ipv4.tcp_slow_start_after_idle 0

# включить aggressive local port reuse (если часто создаются новые короткие соединения)
apply net.ipv4.tcp_tw_reuse 1
