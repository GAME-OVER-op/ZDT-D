...
/// suspect rechecks triggered by runtime failures stay active (they are
/// serialized at the dial level and their fresher results win on import).
...
async fn peer_loop(state: crate::AppState) {
    let instances_dir = PathBuf::from(state.args.api_dir.trim())
        .join("t2s")
        .join("instances");
    let token = state.api.token.clone();
    ...
    loop {
        ...
        for meta in scan_instance_files(&instances_dir, now_ts).await {
            ...
            let Some(value) =
                http_get_json(web, "/api/v1/backends", token.as_deref(), HTTP_TIMEOUT).await
            else {
                continue;
            };
        ...
        // Peers that stopped answering were simply not re-added above; the
        // table is rebuilt from scratch every scan.
...
/// Minimal HTTP/1.1 GET for loopback JSON APIs; avoids pulling in an HTTP
/// client dependency for a fixed set of trusted peer endpoints.
