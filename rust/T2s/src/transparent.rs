...
pub async fn bind_tcp_listener(addr: SocketAddr, tproxy_enabled: bool) -> Result<TcpListener> {
    ...
    unsafe {
        ...
        let (level, opt) = if bind_addr.is_ipv4() {
            (libc::SOL_IP, 19) // IP_TRANSPARENT
        } else {
            (libc::SOL_IPV6, 75) // IPV6_TRANSPARENT
        };
        ...
        if rc != 0 {
            return Err(std::io::Error::last_os_error()).context("setsockopt IP_TRANSPARENT/IPV6_TRANSPARENT");
        }
    }
...
pub async fn bind_tcp_listener(addr: SocketAddr, tproxy_enabled: bool) -> Result<TcpListener> {
    if tproxy_enabled {
        return Err(anyhow::anyhow!("TPROXY transparent sockets are supported only on Linux/Android"));
    }
...
pub fn get_original_dst(stream: &TcpStream) -> Result<SocketAddr> {
    ...
    // Try IPv4 NAT REDIRECT original destination.
    unsafe {
...
pub fn get_transparent_dst(stream: &TcpStream, listen_addr: &str, listen_port: u16, tproxy_enabled: bool) -> Result<SocketAddr> {
    if !tproxy_enabled {
        return get_original_dst(stream);
    }
    ...
    // Compatibility fallback: if a REDIRECT-style connection reaches a TPROXY-enabled
    // process on the same port, keep the old SO_ORIGINAL_DST path. Do not silently
    // use the listener address as target because that can create a self-loop.
