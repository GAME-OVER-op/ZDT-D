...
use std::{net::{IpAddr, Ipv4Addr, SocketAddr}, os::unix::io::{AsRawFd, RawFd}, sync::Arc, time::Duration};
...
pub async fn run_udp_tproxy(state: AppState) -> Result<()> {
    ...
    loop {
        ...
        tokio::spawn(async move { if let Err(e) = handle_udp_packet(st, udp_send, pkt).await { tracing::debug!("udp packet handling failed: {:#}", e); } });
    }
}
...
async fn handle_udp_packet(state: AppState, udp_sock: Arc<AsyncFd<AsyncUdpSocket>>, pkt: UdpPacket) -> Result<()> {
    ...
    match action {
        Some(rules::Action::Drop) | Some(rules::Action::Reset) | Some(rules::Action::Wait) => { state.stats.inc_policy_drop(); return Ok(()); }
    ...
    if let Some((idx, backend, auth)) = { let mut b = state.backends.lock(); b.select_udp_with_auth(global_auth(&state), true) } {
        match udp_socks_once(&state, udp_sock.clone(), &pkt, idx, backend, auth).await {
            Ok(()) => return Ok(()),
            Err(e) => tracing::debug!("udp via socks backend {} failed, falling back direct: {:#}", backend, e),
        }
    }
...
async fn udp_socks_once(state: &AppState, udp_sock: Arc<AsyncFd<AsyncUdpSocket>>, pkt: &UdpPacket, _idx: usize, backend: SocketAddr, auth: Option<(String, String)>) -> Result<()> {
    if state.args.wrapped_socks_addr()?.is_some() { return Err(anyhow::anyhow!("UDP ASSOCIATE through wrapped SOCKS is unsupported")); }
    ...
    let enc = socks5::encode_udp_packet(socks5::TargetAddr::Ip(pkt.original_dst), &pkt.data)?;
    udp.send_to(&enc, relay).await.context("send socks udp packet")?;
    let mut buf = vec![0u8; 65535];
    let (n, _from) = tokio::time::timeout(Duration::from_secs(3), udp.recv_from(&mut buf)).await.context("socks udp response timeout")?.context("recv socks udp response")?;
...
async fn udp_direct(udp_sock: Arc<AsyncFd<AsyncUdpSocket>>, pkt: UdpPacket) -> Result<()> {
    let outbound = tokio::net::UdpSocket::bind(if pkt.original_dst.is_ipv4() { "0.0.0.0:0" } else { "[::]:0" }).await.context("bind direct udp outbound")?;
    outbound.send_to(&pkt.data, pkt.original_dst).await.context("send direct udp")?;
    let mut buf = vec![0u8; 65535];
    let (n, from) = tokio::time::timeout(Duration::from_secs(3), outbound.recv_from(&mut buf)).await.context("direct udp response timeout")?.context("recv direct udp response")?;
