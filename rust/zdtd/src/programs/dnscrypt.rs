...
use crate::{iptables::{caps, iptables_tproxy}, shell::{self, Capture}, xtables_lock};
...
fn apply_dns_iptables(listen_port: u16) -> Result<()> {
    ...
    // Keep the existing TPROXY integration intact; only the DNS interception
    // scheme above is restored to the original implementation.
